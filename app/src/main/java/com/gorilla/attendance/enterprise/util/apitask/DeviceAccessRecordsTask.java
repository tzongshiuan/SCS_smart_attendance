...
public class DeviceAccessRecordsTask extends AsyncTask<Object, Integer, RecordsReplyModel> {

    public static final String TAG = "DeviceAccessRecordsTask";

    IDeviceAccessRecordsListener callback;
    ...
    protected RecordsReplyModel doInBackground(Object... params) {
        ...
        callback = (IDeviceAccessRecordsListener) params[5];
...
//            ArrayList<UserClockBean> attendanceBean = DatabaseAdapter.getInstance(context).getUserClockByModuleRecordMode(Constants.MODULES_ACCESS, Constants.RECORD_MODE_RECORD, Constants.MODULES_ATTENDANCE_ACCESS);
            ...
            LOG.D(TAG,"DeviceAccessRecordsTask attendanceBean = " + attendanceBean);
            if(attendanceBean != null){
                LOG.D(TAG,"DeviceAccessRecordsTask attendanceBean.size() = " + attendanceBean.size());
                for(int i = 0 ; i < attendanceBean.size() ; i++){
                    ...
                    try {
                        ...
                        LOG.D(TAG,"DeviceAccessRecordsTask dateString  = " + dateString);
    ...
    protected void onPostExecute(RecordsReplyModel result) {
        ...
        if(callback != null)
            callback.onDeviceAccessRecords(result);
    }
